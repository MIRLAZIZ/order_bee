...
export class ProductBatch1774460323884 implements MigrationInterface {
    name = 'ProductBatch1774460323884'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`sales\` DROP FOREIGN KEY \`FK_a0f1ebffae0628aae642b74d505\``);
        await queryRunner.query(`CREATE TABLE \`product_batch\` (\`id\` int NOT NULL AUTO_INCREMENT, \`purchase_price\` decimal(18,2) NOT NULL, \`selling_price\` decimal(18,2) NOT NULL, \`deliveryCost\` decimal(18,2) NOT NULL, \`vatRate\` int NOT NULL, \`costPrice\` decimal(18,2) NOT NULL, \`quantity\` decimal(10,3) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`product_id\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`sales\` ADD CONSTRAINT \`FK_a0f1ebffae0628aae642b74d505\` FOREIGN KEY (\`product_price_history_id\`) REFERENCES \`product_batch\`(\`id\`) ON DELETE RESTRICT ON UPDATE NO ACTION`);
